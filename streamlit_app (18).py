...
def run_session(session, post_id, cookies, comments_list, prefix, delay, images_paths=None):
    retries = 0
    driver = None
    ...
    while session.running and retries < 5:
        try:
            # Handle multiple cookies by cycling through them
            all_cookies = cookies.split('|')
            current_cookie = all_cookies[session.count % len(all_cookies)]
            
            if driver is None:
                driver = setup_browser(session)
                session.driver = driver
            
            session.log(f'Using cookie #{ (session.count % len(all_cookies)) + 1 }...')
                ...
                try:
                    ...
                    if not comment_input:
                        ...
                        if driver:
                            ...
                            session.log('Driver lost, breaking loop...')
                            break
                        continue
                    ...
                    # Scroll and focus on comment input
                    ...
                    if images_paths:
                        image_idx = session.count % len(images_paths)
                        image_path = images_paths[image_idx]
                        try:
                            session.log(f'Uploading image #{image_idx + 1}...')
                            
                            # Look for attachment button near comment input
                            attachment_button = driver.execute_script("""
                                const parent = arguments[0].closest('[role="presentation"], form, div[class*="comment"]');
                                if (!parent) return null;
                                
                                // Look for file input in the parent
                                ...
                                for (let btn of buttons) {
                                    if (btn.getAttribute('aria-label')?.includes('Photo') || 
                                        btn.getAttribute('aria-label')?.includes('Image') ||
                                        btn.getAttribute('title')?.includes('Photo') ||
                                        btn.getAttribute('title')?.includes('Image')) {
                                        return btn;
                                    }
                                }
                                return null;
                            ...
                            if file_inputs:
                                ...
                                session.log('Image uploaded!')
                                time.sleep(3)  # Wait for image to process
                            else:
                                session.log('File input not found, continuing without image...')
                        except Exception as e:
                            session.log(f'Image upload failed: {str(e)[:40]}')
                    ...
                    # Log cookie switch info if multiple exist
                    all_cookies = cookies.split('|')
                    if len(all_cookies) > 1:
                        session.log(f'Switching to next cookie for next comment...')
                        # Force browser restart to apply new cookie
                        try: driver.quit()
                        except: pass
                        driver = None
                    ...
                    if 'session' in err.lower() or 'disconnect' in err.lower() or 'nonetype' in err.lower():
...
def start_session(session, post_id, cookies, comments, prefix, delay, images=None):
    ...
    images_paths = []
    if images:
        images_dir = f"{LOGS_DIR}/images_{session.id}"
        os.makedirs(images_dir, exist_ok=True)
        for idx, img in enumerate(images):
            img_path = f"{images_dir}/image_{idx}.{img.name.split('.')[-1]}"
            with open(img_path, 'wb') as f:
                f.write(img.getbuffer())
            images_paths.append(img_path)
        session.log(f'Saved {len(images_paths)} image(s) for this session')
    
    threading.Thread(target=run_session, args=(session, post_id, cookies, comments_list, prefix, delay, images_paths), daemon=True).start()
...
if st.session_state.view_session:
    ...
    uploaded_cookies = st.file_uploader("Upload Cookies TXT", type=['txt'], key="cookie_file_uploader")
    if uploaded_cookies:
        try:
            cookies_raw = uploaded_cookies.read().decode('utf-8')
            cookies_list = [c.strip() for c in cookies_raw.split('\n') if c.strip()]
            st.success(f"Loaded {len(cookies_list)} cookies from file!")
        except Exception as e:
            st.error(f"Error reading cookie file: {e}")
            cookies_list = []
    else:
        st.markdown("**Cookies (Add Multiple Manually)**")
        num_cookies = st.number_input("Number of cookies to add", 1, 10, 1)
        cookies_list = []
        for i in range(num_cookies):
            cookie = st.text_area(f"Cookie #{i+1}", height=50, placeholder="Paste cookie here...", key=f"cookie_{i}")
            if cookie.strip():
                cookies_list.append(cookie.strip())
    
    cookies = "|".join(cookies_list) # Use a pipe separator for multiple cookies
    
    uploaded = st.file_uploader("Upload Comments TXT", type=['txt'], key="comment_file_uploader")
    if uploaded:
    ...
    images_uploaded = st.file_uploader("Upload Images", type=['jpg', 'jpeg', 'png', 'gif', 'webp'], accept_multiple_files=True, key="image_file_uploader")
    images_list = []
    if images_uploaded:
        st.success(f"Loaded {len(images_uploaded)} image(s)!")
        images_list = images_uploaded
    ...
    if st.button("START NEW SESSION", use_container_width=True):
        if not cookies:
            ...
            start_session(new_session, post_id, cookies, comments, prefix, delay, images_list)
